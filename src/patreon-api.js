...
const getPledgesAndFollowLinks = function (url, pledges = []) {
  ...
  return fetch(url)
    ...
    .then(function(body) {
      ...
      return body.links && body.links.next 
        ? getPledgesAndFollowLinks(`${body.links.next}&type=pledge&access_token=${accessToken}`, pledges) 
        : pledges
      })
}
...
const saveToFile = async (pledges) => {
...
//   .then(() => getPledgesAndFollowLinks(`https://www.patreon.com/api/oauth2/api/campaigns/1137737/pledges` +
